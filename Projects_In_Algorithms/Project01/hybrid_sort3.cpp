...
std::vector<int> h_sort3(std::vector<int>& nums, int h);
std::vector<int> merge3(std::vector<int> inV1, std::vector<int> inV2);
...
std::vector<int> h_sort3(std::vector<int>& nums, int h)
{
  h = (int)sqrt(sqrt(nums.size()));
    ...
    if(nums.size() > h)
    {
      ...
      return merge3(h_sort3(lVec, h), h_sort3(rVec, h));
     }
    else
      {
	insertion_sort(nums);
